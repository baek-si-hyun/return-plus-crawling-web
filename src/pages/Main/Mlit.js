...
import "./CustomDatePicker.css"; // 커스텀 CSS 임포트
  ...
  const fetchData = async (searchKeyword, startDate, endDate) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://returnplus.kr:8888/mlit/?search_keyword=${encodeURIComponent(
          searchKeyword
    ...
    <div className="flex items-center flex-1">
      ...
      <div className="relative w-fit">
        ...
        <DatePicker
          ...
          className="
            block p-2.5 h-11 pl-10 text-sm text-white placeholder-gray-400 bg-gray-700 
            border-t-2 border-b-2 border-l-2 border-r-2 rounded-r-lg 
            outline-none w-60 border-l-gray-600 border-t-gray-600 
            border-b-gray-600 border-r-gray-700 focus:ring-blue-500 focus:border-blue-500
          "
        />
      </div>
      ...
      {status === "idle" && (
        <button
          type="button"
          className="
            rounded-lg text-sm h-11 px-5 py-2.5 font-medium text-white
            focus:outline-none focus:ring-4 focus:ring-blue-800 
            bg-blue-600 hover:bg-blue-700 ml-4
          "
      ...
      {status === "ready" && (
        <div className="flex items-center ml-4">
          <button
            type="button"
            className="
              rounded-lg text-sm px-5 h-11 py-2.5 font-medium text-white
